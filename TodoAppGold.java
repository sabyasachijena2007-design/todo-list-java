...
public class TodoAppGold {
    ...
    // ── FIX: refreshTaskList() — updateStatus() at end ─────────
    ...
    // ── FIX: taskRow() — checkbox updates label in-place, no refreshTaskList() ──
    private JPanel taskRow(Task task, int idx) {
        ...
        check.setSelected(task.done);   // reflects DB state on reload — DO NOT CHANGE
        check.setOpaque(false);
        check.setUI(new GoldCheckUI());

        // Label declared before listener so listener can mutate it in-place
        ...
        check.addActionListener(e -> {
            ...
            // Update label and card color in-place — NO refreshTaskList()
    ...
    private void updateStatus() {
        ...
        statusLabel.setText(done + "/" + total);
    }
